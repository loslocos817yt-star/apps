...
sleep 5
# Convertimos el link a formato 'raw' para descargar solo el código
...
# Paso 4 (12-16s)
echo -e "${C_C}[4/6] Inyectando tablas base (comandos.json y link.json)...${C_0}"
sleep 4
...
# Inyectando comandos.json (La versión v8.0 que cuadramos)
cat <<EOF > comandos.json
{
  "Version": "8.0-CUSTOM_FARMER",
  "comandos": [
    {
      "comando": "help",
...
EOF
echo -e "${C_G}  -> Bases de datos JSON generadas con éxito.${C_0}"
...
cd ..
mkdir -p apps config
echo -e "${C_G}  -> Sectores creados.${C_0}"
...
echo "------------------------------------------------"
echo -e "${C_V}✅ Sistema instalado.${C_0}"
